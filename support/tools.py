...
def get_order_details(order_id):
    try:
        ...
        return {
            ...
            "dilevery_address": order.delivery,
            "ordered_on": order.created_at.strftime("%d %b %Y"),
            "days_science_order": (timezone.now() - order.created_at).days,
...
def check_dilevery_status(tracking_number, carrier):
    default_response = {
        "status": "Unknown",
        "last_location": "Tracking info unavaliavle",
        "last_update": "N/A",
...
def get_costumer_risk_profile(user_id):
    refunds = RefundRequest.objects.filter(user_id=user_id)
    orders = Order.objects.filter(user_id=user_id)

    # refunds of last 90 days
